...
> **Frozen product roadmap, version 2.0.3**

## 1. Roadmap boundary

The current objective ends with the verified, hashed, committed, and pushed v2.0.3 documentation freeze. It does not authorize product implementation. A separate future objective must identify the implementation repository, baseline, branch, and pull-request target before building the smallest complete browser V0.1 through the ordered increments below.
...
The v2.0.3 baseline consists of:

- exactly 13 Markdown files frozen as `emenda-clean-room-v2.0.3-2026-08-21`;
- one documentation-only child commit of v2.0.2 at `6a4ddc65fa9067f94023f87aebe48840e1b88bc2`, with v2.0.1 preserved at `d70b277998a23663ee6befc77dd6bb0da50ebcca` and v2.0.0 at `a1a13607867db8e6eb2ea904f6387ba130f22ce7`;
...
4. **Unified DOM Integration** — implement the one supported light-DOM textarea surface, including paired-input provenance, conservative exposure, exact selection identity, scalar/UTF-16 mapping, approval focus handoff, IME handling, self-mutation consumption, and one-step Undo.
...
6. **Browser Integration** — integrate the supported textarea flow and verify refused surfaces, permissions, storage isolation, teardown, accessibility, authority races, and editing behavior before passing the Browser Integration Gate.
7. **V0.1 Conformance** — commit the final implementation, test and push that exact commit, then commit and push its factual evidence, pass the V0.1 Conformance Gate, verify a clean worktree, and stop.
...
The Provider Gate requires all deterministic checks to pass and one complete sequential `15/15` live qualification whose factual results remain visible. [`docs/ACCEPTANCE.md`](docs/ACCEPTANCE.md) owns the corpus and evidence requirements; the live run is not a future reliability guarantee.
...
Each entry reports only what ran on that device. V0.1 is complete only after the tested implementation commit and its later factual evidence commit are both pushed and verified and the worktree is clean.
